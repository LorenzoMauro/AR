...
class Training:
    def __init__(self,Networks):
        ...
        with tf.name_scope('Summary'):
            ...
            argmax_labels = tf.argmax(tf.cast(conc_labels, tf.int64), axis=1)
            argmax_activity_labels = tf.argmax(tf.cast(conc_activity_labels, tf.int64), axis=1)
            argmax_History_labels = tf.argmax(tf.cast(conc_History_labels, tf.int64), axis=2)
