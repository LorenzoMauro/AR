    ...
    def __init__(self, devices, IO_tool):
        ...
        with tf.name_scope('Inputs'):
            ...
            with tf.name_scope("Now_target"):
                ...
                self.next_labels = tf.placeholder(tf.int32, shape=(None, config.Batch_size, config.seq_len, None), name="next_labels")
    ...
    def __init__(self, number_of_classes, number_of_activities, Input_manager, device_j, IO_tool):
        ...
        with tf.name_scope('Activity_Recognition_Network'):
            ...
            with tf.name_scope("Now_Target"):
                ...
                now_one_hot_label= tf.one_hot(self.labels, depth = self.out_vocab_size)
            ...
            with tf.name_scope("Next_Target"):
                ...
                next_dec_input = tf.concat([tf.fill([config.Batch_size, 1], IO_tool.dataset.label_to_id['go']), self.next_labels], 1)
                next_one_hot_label= tf.one_hot(self.next_labels, depth = self.out_vocab_size)
                next_dec_embed_input = tf.nn.embedding_lookup(Input_manager.dec_embeddings, next_dec_input)
                next_target_len = tf.ones(shape=(config.Batch_size), dtype=tf.int32)*config.seq_len
            ...
            with tf.name_scope('Now_Decoder'):
                ...
                now_logit, now_softmax, now_predictions, now_one_hot_prediction = train_lstm(encoder_state, now_decoder, now_output_layer, now_dec_embed_input, now_target_len)

            with tf.name_scope('Next_Decoder'):
                now_softmax.set_shape([None, config.seq_len, self.out_vocab_size])
                flat_now = tf.contrib.layers.flatten(now_softmax)
                ...
                next_state = tf.contrib.rnn.LSTMStateTuple(new_C, new_H)
                next_decoder, next_output_layer = decoder_lstm(config.lstm_units)
                next_logit, next_softmax, next_predictions, next_one_hot_prediction = train_lstm(next_state, next_decoder, next_output_layer, next_dec_embed_input, next_target_len)
            

            with tf.name_scope("c3d_classifier"):
                composedVec = tf.concat([self.c3d_out], axis=1)
