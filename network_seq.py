    ...
    def __init__(self, devices):
        ...
        with tf.name_scope('Inputs'):
            ...
            with tf.name_scope("Input"):
                self.input_batch = tf.placeholder(tf.float32, shape=(None, config.Batch_size, config.seq_len, config.frames_per_step, config.out_H, config.out_W, config.input_channels), name="Input")
                self.h_input = tf.placeholder(tf.float32, shape=(None, config.Batch_size, config.seq_len, config.lstm_units), name="Previous_hidden_state")
                self.c_input = tf.placeholder(tf.float32, shape=(None, config.Batch_size, config.seq_len, config.lstm_units), name="Previous_hidden_state")

            with tf.name_scope("Target"):
                self.labels = tf.placeholder(tf.float32, shape=(None, config.Batch_size, config.seq_len, None), name="Target")
                self.activity_labels = tf.placeholder(tf.float32, shape=(None, config.Batch_size, config.seq_len, None), name="Target")
                self.next_labels = tf.placeholder(tf.float32, shape=(None, config.Batch_size, config.seq_len, None), name="next_labels")
                self.multiple_next_labels = tf.placeholder(tf.float32, shape=(None, config.Batch_size, config.seq_len, None), name="next_labels")
    ...
    def __init__(self, number_of_classes, number_of_activities, max_history, Input_manager, device_j):
        ...
        with tf.name_scope('Activity_Recognition_Network'):
            ...
            with tf.name_scope("Target"):
                ...
                self.argmax_labels = tf.argmax(tf.cast(self.labels, tf.int64), axis=1)
                self.argmax_activity_labels = tf.argmax(tf.cast(self.activity_labels, tf.int64), axis=1)
                self.argmax_History_labels = tf.argmax(tf.cast(self.history_labels, tf.int64), axis=2)
            ...
            def C3d(self, input)
                with tf.name_scope('C3d'):
                    ...
                    with tf.name_scope("Conv"):
                        conv1 = self.conv3d('conv1', input, wc['wc1'], bc['bc1'])
            ...
            def c3d_map_fn(self, input):
                c3d_out = tf.map_fn(self.C3d(x),input)
                print(c3d_out)
                return c3d_out

            with tf.name_scope('c3d_mapfn'):
                self.c3d_out = tf.map_fn(self.c3d_map_fn(x), self.input_batch)
                print(c3d_out)
            ...
            with tf.name_scope("c3d_classifier"):
                composedVec = tf.concat([self.reshape_2_cd], axis=1)
            ...
            with tf.name_scope("Activity_Classifier"):
                self.activity_logit = tf.layers.dense(self.out_cd, config.pre_class)
            ...
            with tf.name_scope("Now_Classifier"):
                input_now= tf.concat([decoder_output, self.activity_logit], axis=1)
            ...
            with tf.name_scope('history_decoder'):
                history_input = tf.concat([decoder_output, self.activity_logit, self.out_L], axis=1)
                history_input = tf.expand_dims(history_input, 1)
    ...
    def __init__(self,Networks):
        ...
        with tf.name_scope('Loss'):
            ...
            with tf.name_scope("Global_Loss"):
                ...
                # total_loss = (c3d_recall)*(activity_recall*(now_recall * history_loss_sum + (1-now_recall)*now_loss_sum) + (1 - activity_recall)*activity_loss_sum) + (1 - c3d_recall) * c3d_loss_sum + auto_enc_loss_sum
                total_loss = (c3d_recall)*(activity_loss_sum) + (1 - c3d_recall) * c3d_loss_sum + auto_enc_loss_sum
        ...
        with tf.name_scope('Summary'):
            ...
            for Net in Networks:
                if j == 0:
                    conc_predictions_activity = Networks[Net].activity_predictions
                    conc_predictions_c3d = Networks[Net].predictions_c3d
                    conc_predictions_Lstm = Networks[Net].predictions_Lstm
                    conc_predictions_History = Networks[Net].history_predictions
                    conc_labels = Networks[Net].labels
                    conc_activity_labels = Networks[Net].activity_labels
                    conc_History_labels = Networks[Net].history_labels
                else:
                    conc_predictions_activity = tf.concat([conc_predictions_activity, Networks[Net].activity_predictions], axis=0)
                    ...
                    conc_predictions_History = tf.concat([conc_predictions_History, Networks[Net].history_predictions], axis=0)
                    ...
                    conc_History_labels = tf.concat ([conc_History_labels, Networks[Net].history_labels], axis=0)
                j += 1


            # tf.summary.histogram("labels_target", argmax_labels)
            # tf.summary.histogram("Now_classification", conc_predictions_Lstm)
            tf.summary.histogram("c3d_classification", conc_predictions_c3d)
            # tf.summary.histogram("History_target", argmax_History_labels)
            # tf.summary.histogram("Now_classification", conc_predictions_History)
            # tf.summary.histogram("activity_target", argmax_activity_labels)
            tf.summary.histogram("activity_classification", conc_predictions_activity)
            ...
            tf.summary.scalar('activity_Loss', activity_loss_sum)
            ...
            tf.summary.scalar('activity_recall', activity_recall)
